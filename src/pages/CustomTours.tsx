...
import { customToursApi, clientsApi } from '../services/api';
...
const CustomTours = () => {
  ...
  const [editingTour, setEditingTour] = useState<CustomTour | null>(null);
  const [formData, setFormData] = useState<CustomTour>({
    date: format(new Date(), 'yyyy-MM-dd'),
    product_category: 'Custom Tour',
    product_subcategory: 'Cultural Tour',
    num_pax: 1,
    price_per_pax: 0,
    income: 0,
    other_income: 0,
  ...
  const fetchTours = async () => {
    try {
      const response = await customToursApi.getAll();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTour?.id) {
        await customToursApi.update(editingTour.id, formData);
      } else {
        await customToursApi.create(formData);
      }
      fetchTours();
      closeModal();
    } catch (error) {
  ...
  const handleEdit = (tour: CustomTour) => {
    setEditingTour(tour);
    setFormData(tour);
    setShowModal(true);
  };
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this tour?')) {
      try {
        await customToursApi.delete(id);
        fetchTours();
      } catch (error) {
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Manage your custom tour bookings and track performance</p>
      </div>

      <div className="card">
        <div className="card-header">
          <h2 className="card-title">Custom Made Tours</h2>
        ...
        {tours.length === 0 ? (
          <div className="empty-state">
            <div>No custom tours yet. Click "New Tour" to get started.</div>
          </div>
        ) : (
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header">
              <h2 className="modal-title">{editingTour ? 'Edit Tour' : 'New Custom Tour'}</h2>
