...
import { multiDayToursApi, clientsApi } from '../services/api';
import { format } from 'date-fns';

interface MultiDayTour {
  id?: number;
  date: string;
...
const MultiDayTours = () => {
  const [tours, setTours] = useState<MultiDayTour[]>([]);
  const [clients, setClients] = useState<any[]>([]);
  const [showModal, setShowModal] = useState(false);
  const [editingTour, setEditingTour] = useState<MultiDayTour | null>(null);
  const [formData, setFormData] = useState<MultiDayTour>({
  ...
  const fetchTours = async () => {
    try {
      const response = await multiDayToursApi.getAll();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTour?.id) {
        await multiDayToursApi.update(editingTour.id, formData);
      } else {
        await multiDayToursApi.create(formData);
      }
      fetchTours();
      closeModal();
    } catch (error) {
  ...
  const handleEdit = (tour: MultiDayTour) => {
    setEditingTour(tour);
    setFormData(tour);
    setShowModal(true);
  };
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this tour?')) {
      try {
        await multiDayToursApi.delete(id);
        fetchTours();
      } catch (error) {
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Manage your multi-day tour bookings and track performance</p>
      </div>
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header">
              <h2 className="modal-title">{editingTour ? 'Edit Tour' : 'New Multi-day Tour'}</h2>
