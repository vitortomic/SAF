...
// Helper function to calculate tour totals
...
router.get('/', async (req, res) => {
  try {
    const tours = await dbAll(`
      SELECT dt.*, c.name as client_name
      FROM daily_tours dt
    ...
    res.status(500).json({ error: 'Failed to fetch daily tours' });
  }
});

// Get daily tour by ID
router.get('/:id', async (req, res) => {
  try {
    ...
    `, [req.params.id]);
    if (!tour) {
      return res.status(404).json({ error: 'Daily tour not found' });
    }
    res.json(tour);
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch daily tour' });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to create daily tour' });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to update daily tour' });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    await dbRun('DELETE FROM daily_tours WHERE id = ?', [req.params.id]);
    res.json({ message: 'Daily tour deleted successfully' });
  } catch (error) {
    res.status(500).json({ error: 'Failed to delete daily tour' });
  }
});
